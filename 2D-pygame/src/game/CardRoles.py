...
from GameEvent import DealsDamageEvent, CardTapped, CardUntapped, PermanentDestroyedEvent, ReceivesDamageEvent, CleanupEvent, AttachedEvent, UnAttachedEvent, AttackerDeclaredEvent, AttackerBlockedEvent, BlockerDeclaredEvent, TokenLeavingPlay, TargetedByEvent
...
class Creature(Role):
    def power():
        def fget(self):
            ...
            modifier2 = sum([t.power for t in self.counters if hasattr(t,"power")])
    ...
    def toughness():
        def fget(self):
            ...
            modifier2 = sum([t.toughness for t in self.counters if hasattr(t,"toughness")])
    ...
    def mustBeBlocked(self):
        return False
    def canAttack(self):
        return (not self.perm.tapped) and (not self.in_combat) and self.perm.continuously_in_play
    ...
    def canTap(self): return self.perm.continuously_in_play
    def shouldDestroy(self):
        return self._damage >= self.toughness
    def __str__(self):
        return "Creature"
...
class Attachment(object):
    def attach(self, target):
        ...
        self.attached_to.enchantments.append(self)
    ...
    def unattach(self):
        if self.attached_to:
            ...
            self.attached_to.enchantments.remove(self)
