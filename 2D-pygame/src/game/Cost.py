from GameEvent import SacrificeEvent
...
class ManaCost(Cost):
    ...
    def __init__(self, cost):
        ...
        self.payment = "0"
        self._X = 0
        self.X = LazyInt(lambda s=self: s._X, finalize=True)
    def compute(self, card, player):
        self.paid = False
        mp = player.manapool
        X = 0
...
class SacrificeCost(Cost):
    ...
    def compute(self, card, player):
        ...
        # XXX Make sure the target can be sacrificed
        # Probably against protections and stuff
        if target.canBeTargetedBy(card):
...
class CounterCost(Cost):
    ...
    def pay(self, card, player):
        ...
        return self.paid
    def reverse(self, card, player):
        if self.paid: card.counters.extend(self.counters[:self.number])
    def __str__(self):
...
class TapCost(Cost):
    def __init__(self, cardtype=None, number=1):
        ...
        #if cardtype: self.cardtype = cardtype.with(lambda c: not c.tapped and c.canTap())
    ...
    def compute(self, card, player):
        ...
        while True:
            ...
            if target in self.targets: prompt = "Target already selected - select again"
            elif target.tapped: prompt = "Target already tapped - select again"
            else:
                prompt = "Select %d %s(s) for tapping"%(self.number-len(self.targets), self.cardtype)
...
class MultipleCosts(Cost):
    ...
    def consolidate(self, costs):
        ...
        if manacost: return [reduce(lambda x,y: x+y, manacost, ManaCost("0"))]+newcost
        else: return costs
    def compute(self, card, player):
        self.paid = False
        for c in self.costs:
...
class LifeCost(Cost):
    ...
    def pay(self, card, player):
        # XXX This needs to trigger something
        player.life -= self.amt
        return True
    def reverse(self, card, player):
        player.life += self.amt
    def __str__(self):
        return "Lose %d life"%self.amt
...
class DiscardCost(Cost):
    ...
    def compute(self, card, player):
        ...
        if self.cardtypes == None:
            ...
            selection = player.hand.get(self.cardtypes)
            if len(selection) < self.number: return False
            result = target.getSelection(selection, self.number, required=False, prompt="Discard cards")
            if not result: return False
            self.discards = result
        return True
    def pay(self, card, player):
