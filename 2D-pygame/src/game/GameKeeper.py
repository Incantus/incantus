    ...
    def process_triggered(self):
        ...
        if len(self.triggered_abilities) > 0:
            ...
            for player, triggered in zip((self.curr_player, self.curr_player.opponent), triggered_sets):
                ...
                results = []
                if len(abilities) > 1:
                    results = player.getSelection(abilities, len(abilities), required=False, isCardlist=False, prompt="Order triggered abilities")
...
class GamePhases(object):
    def __init__(self, gamekeeper, players):
        self.state_map = dict([(p,i) for i, p in enumerate(["BeginTurn",
                     "Main1", "Combat", "Main2", "EndPhase", "Cleanup"])])
        self.game_phases = [self.makeBeginningPhase(gamekeeper), gamekeeper.mainPhase1, gamekeeper.combatPhase, gamekeeper.mainPhase2, gamekeeper.endPhase, gamekeeper.cleanup]
    ...
    def addPhases(self, phases, after_phase=None):
        ...
        elif after_phase == "Cleanup": position=-1
...
class GameKeeper(MtGObject):
    ...
    def init(self, player1, player2):
        ...
        player1.shuffleDeck()
        player2.shuffleDeck()
        for i in range(7): player1.draw()
        for i in range(7): player2.draw()
    ...
    def singleTurn(self):
        ...
        # damage on the stack, fast effects, first strike damage resolves, fast effects,
        # non-first strike damage on the stack, fast effects, damage resolves
        #          post combat main
        #          end step
    ...
    def manaBurn(self):
        self.curr_player.manaBurn()
        self.other_player.manaBurn()
    def checkSBE(self):
        ...
        #    if player.life <= 0: raise GameOver("Player %s loses!"%player.name)
        ...
        for player in players:
            if player.poison >= 10: raise GameOver("Player %s is poisoned"%player.name)
    ...
    def calculateDamage(self, blocking_list):
        ...
        for attacker, blockers in new_combat_list:
            if not attacker.blocked:
                ...
                if "trample" in attacker.keywords: tramplers.append(attacker)
                if len(blockers) == 1:
                    ...
                    damage = self.curr_player.getDamageAssignment([(attacker, blockers)])
                    # XXX I should check whether the attacker can assign damage to blocker
    ...
    def combatPhase(self):
        ...
        if self.curr_player.attackingIntention():
            ...
            attackers = self.curr_player.play.get(Match.isCreature.with(lambda c: c.attacking))
            if attackers:
                # Blocking
                self.setState("Block")
    ...
    def cleanup(self):
        ...
        while True:
            # XXX Currently hard-coded but is probably changed by a card
            numcards = len(self.curr_player.hand)
            diff = numcards - self.curr_player.hand_limit
            if diff > 0:
                result = self.curr_player.getSelection(self.curr_player.hand, diff, prompt="Discard cards")
                if diff > 1: [self.curr_player.discard(card) for card in result]
                else: self.curr_player.discard(result)
