...
from game.GameEvent import AbilityResolved, DealsDamageEvent, ReceivesDamageEvent, DealsCombatDamageEvent, ReceivesCombatDamageEvent, CardEnteredZone, CardLeftZone, CardEnteringZone, CardLeavingZone
...
class Trigger(MtGObject):
    ...
    def setup_trigger(self, ability, trigger_function, match_condition=None):
    ...
    def filter(self, sender, **keys):
        if robustApply(self.match_condition, sender, **keys):
            self.sender = sender
...
class PlayerTrigger(Trigger):
    def filter(self):
        ...
        if self.match_condition(player):
            self.player = player
            self.trigger_function(self)
...
class EnterFromTrigger(CardTrigger):
    def __init__(self, from_zone, to_zone):
        ...
        self.previous_zones = {}
    def setup_trigger(self, ability, trigger_function, match_condition=None):
        super(EnterFromTrigger,self).setup_trigger(ability, trigger_function, match_condition)
        ...
        self.from_Z = getattr(player, self.from_zone)
        self.to_Z = getattr(player, self.to_zone)
        # Get all targets that are already in the from zone
        targets = self.from_Z.get(self.match_condition)
        for t in targets:
            self.previous_zones[t] = self.from_Z
    def filter(self, sender, card):
        if self.match_condition(card):
            if not self.previous_zones.has_key(card):
                if sender == self.from_Z: self.previous_zones[card] = sender
            else:
                if sender == self.to_Z:
