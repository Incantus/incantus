from Ability import Ability, Stackless, PostponeTargeting, StacklessAbility
...
class ClashAbility(StacklessActivatedAbility):
    ...
    def resolve(self):
        ...
        clashing_cards = [controller.library[-1], opponent.library[-1]]
        converted_costs = []
...
def champion(subrole, card, role=isPermanent, subtypes=all_characteristics):
    if type(subtypes) == str: msg = "Champion %s"%subtypes
    else: msg = "Champion %s"%role
    target = Target(target_types=role.with(lambda p: not p == card and p.controller == card.controller and p.subtypes == subtypes), msg=msg)
    champion = TriggeredAbility(card, trigger = EnterTrigger("play"),
            match_condition=SelfMatch(card),
            ability=DoOrAbilityPostponed(card, target=target,
    ...
    champion_return = TriggeredAbility(card, trigger = LeaveTrigger("play", any=True),
            match_condition=SelfMatch(card, lambda x: target.target),
            ability=Ability(card, target=Target(targeting= lambda t=target: t.target),
...
def evoke(subrole, card, cost):
    ...
    evoke = TriggeredAbility(card, trigger = EnterTrigger("play"),
            ...
            ability=StacklessAbility(card, target=Target(targeting="self"),
...
def hideaway(subrole, card, limit=None):
    ...
    hidden = MoveTargetCards(from_zone="library", to_zone="removed", number=1, subset=4, required=True, func = lambda c: c.faceDown())
    hideaway = TriggeredAbility(card, trigger = EnterTrigger("play"),
            match_condition=SelfMatch(card),
            ability=PostponedAbility(card, target=Target(targeting="controller"),
                effects=[hidden,
                         MoveTargetCards(from_zone="library", from_position="top", to_zone="library", to_position="bottom", number=4)])) # the second MoveTargetCards is 4 because when we select them we haven't moved the first one yet - but it will be ignored
