...
from GameEvent import DrawCardEvent, DiscardCardEvent, CardUntapped, PlayerDamageEvent, PlayerStatusChanged, TargetedByEvent
...
from Match import isCreature, isGameObject
...
class Player(MtGObject):
    ...
    def setDeck(self, decklist):
        # Check to make sure the deck is valid
        self.decklist = decklist
    def resetLandActions(self):
        self.land_actions = 1
    def loadDeck(self):
        for cardset, num, name in self.decklist:
            num = int(num)
            for n in range(num):
                card = CardLibrary.createCard(cardset, name, self)
                card.controller = card.owner  # XXX If this is commented, then the cost calculation breaks for playing spells (since the controller isn't defined as yet)
    ...
    #    for card in self.library[-number:][::-1]:
    #        self.moveCard(card, from_location=self.library, to_location=self.hand)
    #        self.send(DrawCardEvent())
    ...
    def getMainAction(self):
        ...
        num_added_actions = 5
        action = self.get(prompt="Play Sorceries, Instants or Activated Abilities")
    ...
    def declareAttackers(self):
        ...
        while num_creatures > 0:
            ...
            #if num_creatures > 0: creature = self.getTarget(isCreature, zone=self.play, spell=False, required=False, prompt=prompt)
            #else: break
        ...
        def convert_gui_action(action):
            ...
            if isGameObject(sel)  and sel.controller == self:
                ...
                if zone == self.play: return PlayAbility(sel)
                    ...
                    return sel.play_action(sel)
                    #if sel.type == "Land": return PlayLand(sel)
                    #elif sel.type == "Instant": return PlayInstant(sel)
                    #elif sel.type == "Sorcery": return PlaySpell(sel)
                    #else: return PlaySpell(sel)
            else: return False
    ...
    def getSelection(self, sellist, numselections, required=True, isCardlist=True, prompt=''):
        def filter(action):
            ...
            return False
        context = {'get_selection': True, 'list':sellist, 'numselections': numselections, "cardlist": isCardlist, 'required': required, 'process': filter}
        sel = self.input(context,"%s: %s"%(self.name,prompt))
        if isinstance(sel, CancelAction): return False
        else: return sel
    def getTarget(self, target_types, zone=None, required=True, spell=True, prompt='Select target'):
        ...
        def filter(action):
            ...
            if zone==None or (hasattr(target,"zone") and (target.zone == zone)):
                #if hasattr(target, "current_role"): current_role = target.current_role
                #else: current_role = target   # XXX This is cheating so we can select abilites on the stack (for countering), which have no current_roles
    ...
    def getDamageAssignment(self, blocking_list, prompt="Assign damage to blocking creatures"):
        def filter(action):
            return action.assignment
        context = {'get_damage_assign': True, 'blocking_list': blocking_list, 'process': filter}
        return self.input(context, "%s: %s"%(self.name,prompt))
    def revealCard(self, cards, msgs=[], title="Reveal cards", prompt=''):
        ...
        context = {'reveal_card': True, 'cardkeys': [c.key for c in cards], 'msgs':msgs, "title":title, 'process': lambda x: True}
        return self.input(context, "%s: %s"%(self.name,prompt))
