...
from GameEvent import PlayActionEvent

class LazyInt:
    def __init__(self, func, finalize=False, event=PlayActionEvent()):
        import new
        if not type(func) == new.function: raise Exception("Argument to LazyInt init must be a function")
        self._func = func
