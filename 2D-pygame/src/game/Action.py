...
class PlayAction(Action):
    ...
    def perform(self, player):
        ...
        if numabilities > 1:
            ...
            abilities = [(str(a), i) for i, a in enumerate(self.card.current_role.abilities)]
            # XXX Must use the index of the ability, since we can't pickle abilities for network games
            index = player.getSelection(abilities, 1, isCardlist=False,required=False,prompt="%s: Select ability"%self.card)
            if index is False: return False
            ability = self.card.current_role.abilities[index]
        else: ability = self.card.current_role.abilities[0]

        if not ability or ability.is_limited(): return False
        ...
        if success:
            print "%s: %s - %s"%(player.name,self.card, ability)
            if ability.needs_stack():
                ...
                del ability
            player.send(PlayActionEvent(), card=self.card)
...
class PlayLand(PlayAction):
    def perform(self, player):
        print "%s: %s"%(player.name,self.card)
    ...
    def perform(self, player):
        ...
        if mana_abilities:
            if len(mana_abilities) > 1:
                ...
                index = player.getSelection(abilities, 1, isCardlist=False, required=False, prompt="%s - Mana abilities"%self.card)
                if index is False: return False
